...
export default function MapMarkers({
      ...
      const onMoveEnd = () => {
        ...
        setTimeout(() => {
          ...
          if (marker) {
            ...
            setActivePopupId(activeSecretId); // Also set active popup state
          }
  ...
  useEffect(() => {
    let frameId;
    let lastUpdate = 0;
    const THROTTLE_MS = 100; // Only update viewport every 100ms
  ...
  // Simplified cluster icon with subtle glow - NO ANIMATIONS for performance
  const getClusterIcon = (count, clusterType = 'default') => {
    ...
    // Simplified color scheme with glow
  ...
  const clusteredMarkers = useMemo(() => {
    ...
    // Pre-filter visible secrets
    const visible = secrets.filter(s => bounds.contains([s.lat, s.lng]));

    // Show individual markers at high zoom
    if (zoomLevel >= CLUSTER_OFF_ZOOM) {
      // Return ALL visible secrets as individual markers
      return visible.map((s) => ({
        position: [s.lat, s.lng],
        secrets: [s],
        isCluster: false,
        secretId: s.id, // Keep track of the actual secret
      }));
    }
  ...
  // Cache echoed and my secrets outside render
  const echoedSecrets = useMemo(() => 
    JSON.parse(localStorage.getItem("nodded_secrets") || "[]"),
    [activePopupId] // Only recalculate when popup changes
  );
  ...
  return (
    <>
      {clusteredMarkers.map((cluster, idx) => {
        ...
        // Individual marker rendering - EVERY secret gets its own marker at high zoom
          ...
          >
            ...
            > 
              ...
              >
                ...
                {/* Find secrets at the same location for threading */}
                {(() => {
                  ...
                  const currentSecret = secretsAtLocation[currentIndex] || s;
